...
{
    //autoware_cmd.linear.x = abs(msg->linear.x); 
    //msg->Twist_cmd.twist.linear.x?? check!!
    //TwistStamped->twist.linear.x
    //Twwist      ->linear.x　メッセージ形式の確認!!  
    //TwistStamped ----> header + twist (geometry_msgs::Twist)
...
int main(int argc, char* argv[]){
    ...
    /*ros*/
    ros::init(argc, argv, "unitree_controled_by_autoware");
    ...
    while (ros::ok()){
        ...
        if (flag == 1){
            ...
            SendHighROS.yawSpeed = (float)autoware_cmd.angular.z * yawspeed;
            SendHighROS.footRaiseHeight = -0.25f;
